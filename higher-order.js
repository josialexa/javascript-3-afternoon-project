...
let totalPop = (tot, v, i) => { return tot += v };
let totalPopulation = populations.reduce(totalPop);
...
let monChecker = (v, i) => { return v.CP > 200 };
let myStrongest = monstersInYourPocket.filter(monChecker);
...
let orderTotals = orders.map((v, i) => {
...
let bobsTotal = purchases.reduce((t, v, i) => {
  return v.owner == 'Bob' ? t + v.price : t;
}, 0);